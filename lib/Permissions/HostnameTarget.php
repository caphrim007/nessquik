...
* @author Tim Rupp
*/
class Permissions_HostnameTarget extends Permissions_Abstract {
	...
	*/
	public function add($target) {
		...
		try {
			...
			$db->insert('permissions_hostname', $data);
			return true;
	...
	*/
	public function delete($id) {
		...
		try {
			...
			$db->delete('permissions_hostname', $where);
			return true;
		} catch (Exception $error) {
			throw new Permissions_Exception($error->getMessage());
		}
	...
	*/
	public function get($resource = null, $page = 1, $limit = 15) {
		...
		$sql = $db->select()
			->from('permissions_hostname', array('permission_id' => 'id', 'permission_resource' => 'resource'));
