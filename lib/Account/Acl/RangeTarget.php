...
* @author Tim Rupp
*/
class Account_Acl_RangeTarget extends Account_Acl_Abstract {
	...
	*/
	public function isAllowed($resource) {
		...
		if (!is_array($resource)) {
			$resource = Ip::range2cidr($resource);
		}

		$sql = $db->select('permissions_address.id')
			...
			->join('permissions_address',
				sprintf('%s.%s = %s.%s', 
					$db->quoteIdentifier('roles_permissions'),
					$db->quoteIdentifier('permission_id'),
					$db->quoteIdentifier('permissions_address'),
